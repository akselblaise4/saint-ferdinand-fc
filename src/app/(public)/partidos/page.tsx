...
const STADIUM_IMAGE = "https://lh3.googleusercontent.com/aida-public/AB6AXuCvrliC0rGqMa1oy_kB5nBlntgwEyUMTJJJN5-P-5NmScXVUuCOAVgu49ls7pZdKvXEELCY-EUMWBIxHzyp1IwR560CFu052oFG0-Fx4G9OziVtHWPe8yb8-i6P_wNMb1Hzv5PrslnygsVG2I42Itk5daRQAxZUG_O1dQDngwBGkqTrYJalbzijobzgAnRd_VnKI7Qc6pJWLId4xB_NbZKvE-Y2QqvVFdPm52CSBF_F_PirDuypy7nk56Tl3IowTlFjEqqBQpHshGE";
const SAINTS_CREST = "https://lh3.googleusercontent.com/aida-public/AB6AXuCLbNxiJiG6GJ4EizBtOaM4q4vZLe_BTHAuSryW7d4vqKgrjfw5_GQGCINzzBdm3I9rrR8HPjSpKrdqh_Se4NdUI7R4YM9BKege9x6TRFj2Fr1dwpu3vqRyzF3iFAFiD_H_vH6YIWDOv6CG0wVy9L1pR_bYFGzpYyeVwh3I89WrJKMnHWMCDBa0wnbbqyeAUqJUTfHwIH_a95H17q_3e8SkUUlt3rOUwG706MHV3aoZQXQDLUiGmU62Vkd0K04ZL4h00AP52_EcHnY";
...
export default function PartidosPage() {
  ...
  const regularMatches = useMemo(
    () => matches.filter((m) => !m.isPlayoff).sort((a, b) => (a.dateTimestamp || 0) - (b.dateTimestamp || 0)),
    [matches]
  );
    ...
    <>
      <section className="relative h-[819px] flex items-center justify-center overflow-hidden bg-on-surface">
        <div className="absolute inset-0">
          <div className="w-full h-full bg-cover bg-center grayscale contrast-125" style={{ backgroundImage: `url('${STADIUM_IMAGE}')` }} />
        </div>
            ...
            <>
              ...
              <div className="flex flex-col md:flex-row items-center justify-between gap-12 md:gap-0">
                <div className="flex flex-col items-center flex-1">
                    <div className="w-32 h-32 md:w-48 md:h-48 mb-6 p-4 bg-white/10 backdrop-blur-md border border-white/20 rounded-full flex items-center justify-center">
                ...
                <div className="flex flex-col items-center">
                  ...
                  <div className="text-white space-y-1">
                    {nextMatch.venue && <p className="font-label-lg text-label-lg tracking-[0.3em] uppercase">{nextMatch.venue}</p>}
                    {nextMatch.date && (
                      <p className="font-headline-md text-headline-md font-bold">
                        {new Date(nextMatch.date).toLocaleDateString("es-ES", { day: "numeric", month: "short", year: "numeric" }).toUpperCase()}
                      </p>
                    )}
                  </div>
                </div>
      ...
      <main className="pt-20">
        <section className="py-section-gap bg-surface">
          <div className="max-w-desktop mx-auto px-margin-mobile md:px-margin-desktop">
            <div className="flex flex-col md:flex-row justify-between items-end mb-16 gap-8">
              <div>
                <h2 className="font-display-lg text-display-lg text-on-surface uppercase tracking-tighter mb-2">Calendario de Partidos</h2>
                <p className="text-on-surface-variant font-body-lg text-body-lg">{regularMatches.length} partidos en fase regular. Filtra por estado.</p>
              </div>
              <div className="flex flex-wrap gap-4">
                {filters.map((f) => (
                  <button
                    ...
                    className={`px-6 py-2 font-label-sm uppercase tracking-wider transition-colors ${filter === f.key ? "bg-primary text-on-primary" : "bg-white border border-secondary-container text-on-surface-variant hover:border-primary"}`}
                  >
                    {f.label}
                  </button>
                ))}
            ...
            {playoffMatches.length > 0 && (
              <div className="mb-12">
                <h3 className="font-headline-lg text-headline-lg uppercase tracking-tighter mb-6 text-primary">Playoffs</h3>
                <div className="space-y-4">
                  {playoffMatches.map((m) => {
                    ...
                    return (
                      <div key={m.id} className="bg-surface-container-lowest border border-surface-container-high hover:border-primary transition-colors p-6 md:p-8">
                        <div className="flex flex-col md:flex-row items-center gap-6">
                          <div className="w-full md:w-40 shrink-0">
                            <p className="font-label-sm text-on-surface-variant uppercase tracking-widest">
                              {m.date ? new Date(m.date).toLocaleDateString("es-ES", { day: "numeric", month: "short", year: "numeric" }) : "—"}
                            </p>
                          ...
                          <div className="flex-1 flex items-center justify-center gap-4 md:gap-8">
                            <div className="flex items-center gap-3 flex-1 justify-end">
                              <span className="font-headline-md text-headline-md uppercase text-right">{isHome ? "ST. FERDINAND" : m.homeTeam.name}</span>
                            ...
                            <div className="flex items-center gap-3">
                              ...
                              <span className="px-4 py-1 bg-surface-container text-on-surface-variant font-label-sm uppercase tracking-widest">{m.finished ? "FT" : "VS"}</span>
                            ...
                            <div className="flex items-center gap-3 flex-1">
                              ...
                              <span className="font-headline-md text-headline-md uppercase">{isHome ? m.awayTeam.name : "ST. FERDINAND"}</span>
                            </div>
                          </div>
                        </div>
                      </div>
                    ...
                    >
                      <div className="flex flex-col md:flex-row items-center p-6 md:p-8 gap-6 md:gap-8">
                        <div className="w-full md:w-48 shrink-0">
                          ...
                          <p className="font-headline-md text-headline-md">{m.localHour ? `${m.localHour}:00` : "—"}</p>
                        </div>
                        <div className="flex-1 flex items-center justify-between gap-4">
                          <div className="flex items-center gap-3 md:gap-6 flex-1 justify-end">
                            <span className="font-headline-md text-headline-md uppercase text-right">{isHome ? "ST. FERDINAND" : m.homeTeam.name}</span>
                            <div className="w-12 h-12 bg-surface-container-low p-2 flex items-center justify-center">
                              <div className="w-full h-full bg-contain bg-center bg-no-repeat" style={{ backgroundImage: `url('${SAINTS_CREST}')` }} />
                            </div>
                          </div>
                          <div className="flex items-center gap-3">
                            {m.finished ? (
                          ...
                          <div className="flex items-center gap-3 md:gap-6 flex-1">
                            <div className="w-12 h-12 bg-surface-container-low p-2 flex items-center justify-center">
                              <div className="w-full h-full bg-contain bg-center bg-no-repeat opacity-50" style={{ backgroundImage: `url('${SAINTS_CREST}')` }} />
                            </div>
                            <span className="font-headline-md text-headline-md uppercase">{isHome ? m.awayTeam.name : "ST. FERDINAND"}</span>
                          </div>
                        </div>
                        <div className="w-full md:w-64 flex justify-end gap-3 shrink-0">
                          <Link href="/copa" className="flex-1 md:flex-none bg-surface-container-highest text-on-surface font-label-sm uppercase py-3 px-5 text-center hover:bg-primary hover:text-on-primary transition-all tracking-wider">
                            Detalles
                          </Link>
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-gutter items-start">
              <div className="md:col-span-8 bg-surface-container-lowest border border-surface-container p-8 md:p-10">
                <div className="flex justify-between items-center mb-8 md:mb-10">
                  <h3 className="font-headline-lg text-headline-lg uppercase tracking-tighter">Clasificación — {saints?.group || "Torneo"}</h3>
                  <Link href="/copa" className="font-label-sm text-primary uppercase tracking-widest flex items-center gap-2 hover:opacity-80">
                  ...
                  <table className="w-full text-left border-collapse">
                    ...
                    <tbody className="font-body-md text-body-md">
                      {standings.slice(0, 8).map((s) => {
                        ...
                        return (
                          <tr key={s.id} className={`border-b border-surface-container ${isSaints ? "bg-primary-container/5" : ""}`}>
                            ...
                            <td className="py-4 px-2 text-center">{s.stats ? `${s.stats.goalDiff > 0 ? "+" : ""}${s.stats.goalDiff}` : "0"}</td>
              ...
              <div className="md:col-span-4 space-y-6">
                <h3 className="font-headline-lg text-headline-lg uppercase tracking-tighter mb-4">Resultados Recientes</h3>
                {recentResults.length === 0 ? (
                  ...
                  recentResults.map((m) => {
                    ...
                    const scorers = m.details?.list?.filter((e) => e.type === "goal" && !e.cardColor).map((e) => e.playerName).filter(Boolean) ?? [];
                    return (
                      <div key={m.id} className={`bg-surface-container-high p-6 border-l-4 ${borderColor}`}>
                        <div className="flex justify-between items-start mb-4">
                          <span className="font-label-sm uppercase tracking-widest text-on-surface-variant">{m.phase || `Jornada ${m.round || ""}`}</span>
                        ...
                        <div className="flex items-center justify-between">
                          <div className="flex-1 text-center">
                            ...
                            <span className="font-label-sm uppercase block text-on-surface-variant">{isHome ? "STF" : m.homeTeam.name.slice(0, 3).toUpperCase()}</span>
                          </div>
                          <div className="px-4 flex items-center gap-2">
                            <span className={`font-display-lg text-display-lg leading-none ${isWin ? "text-primary" : ""}`}>{saintScore ?? "?"}</span>
                          ...
                          <div className="flex-1 text-center">
                            ...
                            <span className="font-label-sm uppercase block text-on-surface-variant">{isHome ? m.awayTeam.name.slice(0, 3).toUpperCase() : "STF"}</span>
                          </div>
                        </div>
                        {scorers.length > 0 && (
                          <div className="mt-4 pt-4 border-t border-secondary-container">
                            <p className="font-body-md text-sm text-on-surface-variant italic">Goles: {scorers.join(", ")}</p>
                          </div>
                        )}
                      </div>
                    );
                ...
                <Link href="/copa" className="w-full block text-center py-4 bg-white border border-secondary-container font-label-lg uppercase tracking-widest hover:bg-on-surface hover:text-surface transition-all">
                  Ver Todos
                </Link>
              </div>
            </div>
        ...
        <section className="py-section-gap bg-primary-container text-on-primary overflow-hidden relative">
          ...
          <div className="max-w-desktop mx-auto px-margin-mobile md:px-margin-desktop relative z-10 flex flex-col items-center text-center">
            <h2 className="font-display-lg text-display-lg uppercase tracking-tighter mb-6">No te pierdas ningún momento</h2>
            <p className="font-body-lg text-body-lg mb-10 max-w-2xl opacity-90">Consigue tus abonos o entradas para cada partido. Vive la experiencia Saint Ferdinand.</p>
            <div className="flex flex-wrap justify-center gap-6">
              <button className="bg-surface text-primary font-label-lg uppercase px-12 py-4 tracking-widest hover:bg-on-surface hover:text-surface transition-all">Abonos</button>
              <button className="bg-transparent border-2 border-surface text-on-primary font-label-lg uppercase px-12 py-4 tracking-widest hover:bg-surface hover:text-primary transition-all">Entradas</button>
            </div>
          </div>
        </section>
      </main>
